...
- Pure white #FFFFFF solid background. NO scenery. NO accessory props placed on the floor. NO speech bubbles. NO text elements. NO ground-line shadow that looks like a separate object. ONLY the character and an empty white canvas.`;
      ...
      const defaultVariations = archetypePool.slice(0, 9).map((desc, i) =>
        `Design ${i + 1} — ${desc} Full body, brand-colored accent clothing or accessory. PNG on a pure white (#FFFFFF) solid background — NOT grey, NOT cream, NOT off-white.`
      );
      const variations = (Array.isArray(body.variations) && body.variations.length >= 1)
        ? body.variations.slice(0, 9).map((desc, i) => {
            ...
            return hasPrefix ? desc : `Design ${i + 1} — ${desc} ONE single character only. Full body, PNG on a pure white (#FFFFFF) solid background — NOT grey, NOT cream, NOT off-white.`;
          })
        : defaultVariations;
  ...
  if (url.pathname === '/api/mascot-mode-test' && req.method === 'POST') {
    ...
    try {
      ...
      const finalText = `[MODE ${mode} — ${modeHints[mode]}]\n\n${positivePrompt}${negativeTail}\n\n[STRICT: single-subject only — exactly ONE mascot character in the output, no duplicates. Output MUST be a PNG image on a pure white (#FFFFFF) SOLID background — NOT grey, NOT cream, NOT off-white, NOT gradient. No scenery, no studio backdrop, no dark backgrounds. A clean uniform #FFFFFF backdrop is REQUIRED so chroma-key can cleanly separate the subject.]`;
  ...
  if (url.pathname === '/api/assetpack/generate' && req.method === 'POST') {
    try {
      ...
      const {
        ...
        imageEngine = 'gemini',
        openaiKey = '',
        // Mascot size inside the phone/laptop chat-widget mockups (0.30–0.95).
        // Drives the minScale floor in mockup_compose_v2 — bigger ratio means
      ...
      const defaultTasks = [
        ...
        { id: 'expr-happy',        category: 'expressions', label: 'Happy',        transparent: true, prompt: `The SAME mascot character shown in the reference image (same face, same outfit, same art style), now showing a big bright joyful smile, eyes sparkling with happiness, both hands near the cheeks or clasped together. Full body, single character only. PNG on a pure white (#FFFFFF) SOLID background — NOT grey, NOT cream, NOT off-white.` },
        { id: 'expr-empathetic',   category: 'expressions', label: 'Empathetic',   transparent: true, prompt: `The SAME mascot character from the reference image, now with a soft caring gentle expression, head tilted slightly, one hand placed over the heart, eyebrows slightly raised showing understanding and empathy. Full body, single character only. PNG on a pure white (#FFFFFF) SOLID background — NOT grey, NOT cream, NOT off-white.` },
        { id: 'expr-curious',      category: 'expressions', label: 'Curious',      transparent: true, prompt: `The SAME mascot character from the reference image, now with a curious inquisitive expression, head tilted, one finger on chin, wide interested eyes, small question-mark feeling in the air. Full body, single character only. PNG on a pure white (#FFFFFF) SOLID background — NOT grey, NOT cream, NOT off-white.` },
        { id: 'expr-celebrating',  category: 'expressions', label: 'Celebrating',  transparent: true, prompt: `The SAME mascot character from the reference image, now celebrating — both arms raised high in the air, mouth open in cheer, confetti-like energy around but no actual confetti blocking the character. Full body, single character only. PNG on a pure white (#FFFFFF) SOLID background — NOT grey, NOT cream, NOT off-white.` },
        { id: 'expr-apologetic',   category: 'expressions', label: 'Apologetic',   transparent: true, prompt: `The SAME mascot character from the reference image, now with an apologetic expression — small nervous smile, one hand rubbing the back of the head or neck, shoulders slightly hunched, eyes looking up. Full body, single character only. PNG on a pure white (#FFFFFF) SOLID background — NOT grey, NOT cream, NOT off-white.` },
        { id: 'expr-helpful',      category: 'expressions', label: 'Helpful',      transparent: true, prompt: `The SAME mascot character from the reference image, now in a helpful pose — gesturing forward with an open palm offering assistance, warm welcoming smile, slight forward lean. Full body, single character only. PNG on a pure white (#FFFFFF) SOLID background — NOT grey, NOT cream, NOT off-white.` },

        // 6 ACTIONS
        { id: 'act-waving',        category: 'actions',     label: 'Waving',       transparent: true, prompt: `The SAME mascot character from the reference image, now waving one hand in friendly hello gesture, big smile, free hand at side or on hip. Full body, single character only. PNG on a pure white (#FFFFFF) SOLID background — NOT grey, NOT cream, NOT off-white.` },
        { id: 'act-thumbs-up',     category: 'actions',     label: 'Thumbs Up',    transparent: true, prompt: `The SAME mascot character from the reference image, now giving a big thumbs-up gesture with one hand, confident proud smile, the other hand on hip. Full body, single character only. PNG on a pure white (#FFFFFF) SOLID background — NOT grey, NOT cream, NOT off-white.` },
        { id: 'act-typing',        category: 'actions',     label: 'Typing Laptop',transparent: true, prompt: `The SAME mascot character from the reference image, now sitting and typing on a cute small laptop computer balanced on lap or tiny desk, focused friendly expression. The character and the laptop are the only things in the frame. Single character only. PNG on a pure white (#FFFFFF) SOLID background — NOT grey, NOT cream, NOT off-white.` },
        { id: 'act-pointing',      category: 'actions',     label: 'Pointing',     transparent: true, prompt: `The SAME mascot character from the reference image, now pointing excitedly to the side (as if pointing at a slide or chart), big enthusiastic smile, other hand on waist. Full body, single character only. PNG on a pure white (#FFFFFF) SOLID background — NOT grey, NOT cream, NOT off-white.` },
        { id: 'act-presenting',    category: 'actions',     label: 'Presenting',   transparent: true, prompt: `The SAME mascot character from the reference image, now in a presenting pose — one arm extended to the side with open palm showcasing something, the other hand near the body, professional cheerful smile. Full body, single character only. PNG on a pure white (#FFFFFF) SOLID background — NOT grey, NOT cream, NOT off-white.` },
        { id: 'act-tablet',        category: 'actions',     label: 'Holding Tablet',transparent:true, prompt: `The SAME mascot character from the reference image, now holding a cute small tablet device with both hands, looking at it with interested friendly smile. The character and the tablet are the only objects. Single character only. PNG on a pure white (#FFFFFF) SOLID background — NOT grey, NOT cream, NOT off-white.` },

        // FESTIVE (3) — transparent PNG
        { id: 'fest-birthday',     category: 'festive',     label: 'Birthday',     transparent: true, prompt: `The SAME mascot character from the reference image, now wearing a cute little cone-shaped birthday party hat, holding a small birthday cupcake with a lit candle on top, warm joyful celebration expression. Full body, single character only. PNG on a pure white (#FFFFFF) SOLID background — NOT grey, NOT cream, NOT off-white.` },
        { id: 'fest-christmas',    category: 'festive',     label: 'Christmas',    transparent: true, prompt: `The SAME mascot character from the reference image, now wearing a red Santa hat with white fluffy trim, holding a small wrapped Christmas present box, cozy warm smile. Winter holiday feeling. Full body, single character only. PNG on a pure white (#FFFFFF) SOLID background — NOT grey, NOT cream, NOT off-white.` },
...
[STRICT] Square 1:1 canvas filled edge-to-edge. The phone screen content (mascot, play button, text, QR) must be clearly readable. Single mascot only — printed ON the phone screen, NOT a 3D character outside the phone. Mascot illustration matches the reference EXACTLY at part level (eyes, face, body, colors, outline). The PLAY BUTTON must be visible and obviously look like a video player control. Modern, clean composition — no gradients on the screen content, no extra characters, no third elements competing for attention. Output a complete final photograph.`
        },
        {
          ...
          transparent: false,
          prompt: `A square (1:1) photorealistic product photograph of a collectible 3D vinyl toy figurine of the SAME mascot character from the reference image, sitting on a clean surface with ${_esc(PC.scene)} blurred behind.
...
CAMERA: eye-level shot, shallow depth of field, soft directional lighting from the left. Lifestyle product photography style. No people in frame. Subject centered in the square frame.

[STRICT] Square 1:1 canvas filled edge-to-edge. Single figurine only, no duplicates. Figurine matches reference EXACTLY at part level — only the material rendering changes to glossy vinyl. QR sign card readable. No additional text or logos. Output a complete final product photograph.`
        },
        {
          id: 'promo-banner',
          category: 'mockups',
          label: 'Promo Banner',
          transparent: false,
          prompt: `A square (1:1) photorealistic photograph of a vertical roll-up banner standing on the floor inside ${_esc(PC.banner_location)}, with ${_esc(PC.person)}.
...
CAMERA: eye-level shot from about 3 meters, slight angle, soft natural lighting, banner in sharp focus, person and surroundings in shallow depth of field. Banner centered in the square frame with the location visible around it.

[STRICT] Square 1:1 canvas filled edge-to-edge. Mascot appears ONLY printed on the banner — NOT as a 3D character in the real scene. Mascot illustration matches reference EXACTLY at part level. All banner text readable. Output a complete final photograph.`
        },
      );
        ...
        for (const m of fallbackModels) {
          ...
          for (let attempt = 1; attempt <= 2; attempt++) {
            ...
            try {
              ...
              // pure #FFFFFF (not transparent, not cream, not grey) so the
              // client-side chroma-key can deterministically strip the bg.
              const bgHint = '\n\n[BACKGROUND: Output MUST be a plain pure #FFFFFF SOLID white background — NOT grey, NOT cream, NOT off-white, NOT transparent, NOT gradient. DO NOT use black, dark, scenery, studio, or coloured backgrounds. A flat uniform #FFFFFF backdrop is REQUIRED for chroma-key to work.]';
